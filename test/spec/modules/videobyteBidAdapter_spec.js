...
describe('VideoByteBidAdapter', function () {
  ...
  describe('VideoByteBidAdapter Banner', function () {
    beforeEach(function () {
      bidRequest = {
        mediaTypes: {
          banner: {
    ...
    describe('banner spec.isBidRequestValid', function () {
      it('should return false when banner is empty', function () {
        bidRequest.mediaTypes = {banner:{}};
        expect(spec.isBidRequestValid(bidRequest)).to.equal(false);
      });
      it('should return true if all valid banner params are passed', function () {
        bidRequest.mediaTypes = {banner: {sizes: [300, 250]}}
        expect(spec.isBidRequestValid(bidRequest)).to.equal(true);
      });
    })
    describe('banner spec.buildRequests', function () {
      it('should attach request data', function () {
        const requests = spec.buildRequests([bidRequest], bidderRequest);
        const data = JSON.parse(requests[0].data);
        const [width, height] = bidRequest.mediaTypes.banner.sizes[0];
        expect(data.imp[0].banner.format[0].w).to.equal(width);
        expect(data.imp[0].banner.format[0].h).to.equal(height);
  ...
  describe('VideoByteBidAdapter Video', function () {
    let mockConfig;
    beforeEach(function () {
      bidRequest = {
        mediaTypes: {
